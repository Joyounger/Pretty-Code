...
*/
PUBLIC u8 hd_buf[SECTOR_SIZE];
//在规定的时间内,判断主盘的状态;1为可读写，0为不可读写
PUBLIC int is_hd_status(int timeout)
{
	int t = get_ticks();
	int mask = 0x80;
...
//读盘操作
PUBLIC int read_intr(char *buffer)
{
...
//写盘操作
PUBLIC int write_intr(char *buffer)
{
	if(!is_hd_status(HD_TIMEOUT))
	{
		//		printk("hd is not ready\n");
...
//生成控制磁盘的命令(通用)
PUBLIC struct hd_cmd* make_cmd(struct hd_cmd *cmd,u8 features,u8 count,u8 lba_low,u8 lba_mid,u8 lba_high,u8 device,u8 command)
{
/*	if(device> 1)
...
//生成读写nr_sects个扇区的命令
PUBLIC void make_rw_cmd(struct hd_cmd *cmd,int drive,int start_sect,int nr_sects,int flag)
{
	u8 command = flag;
...
//读分区表所在的扇区(1个)
PUBLIC void get_partition_table(int drive,int start_sect,struct partition_table *entry)
{
	struct hd_cmd cmd;
	//读从start_sect开始的1个扇区
...
//打印分区表
PUBLIC void print_hard_disk_partition(struct hard_disk_info *hd_info)
{
	int i;
...
//获取分区表
PUBLIC void partition(int device,int style)
{
	int i;	
