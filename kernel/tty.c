...
/*
PUBLIC int sys_write(char *buf,int len,struct task_struct *p_proc)
{
	tty_write(&tty_table[p_proc->nr_tty],buf,len);
	return 0;
}
*/
PUBLIC int sys_printx(char *s,int len,struct task_struct *p_proc)
{
	const char *p;
	char ch;
	...
	ch = *p;
	if((ch == MAG_CH_PANIC) || (ch == MAG_CH_ASSERT && current < &proc_table[NR_TASKS]))
	{
		disable_int(); //关中断
