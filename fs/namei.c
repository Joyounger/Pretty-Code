...
PUBLIC int match(struct dir_entry *de,char *name,int namelen)
{
	if(!de || !de->inode_num || namelen > NAME_LEN)
		return -1; 
	if(namelen < NAME_LEN && (namelen != strlen(de->file_name))) 
		return -1;
	return strcmp(de->file_name,name,namelen);

}
PUBLIC int find_entry(struct m_inode *dir,char *name,int namelen,struct dir_entry **res_de)
{
	int i,j,m = 0;
	int ret = -1;
	struct dir_entry *de;
...
PUBLIC int open_namei(char *pathname,int mode,int flag,struct m_inode **res_inode)
{
	int namelen;
	char *basename;
	//目录的inode
