...
PRIVATE struct buffer_head *buffer_free_list;
PRIVATE struct task_struct *buffer_wait = NULL;
int NR_BUFFERS = 0;
...
PRIVATE inline void remove_from_queues(struct buffer_head *bh)
{
...
PRIVATE inline void insert_into_queues(struct buffer_head *bh)
{
...
PRIVATE void wait_on_buffer(struct buffer_head *bh)
{
	disable_int();
	while(bh->b_lock)
		sleep_on(&bh->b_wait);
...
PRIVATE inline void lock_buffer(struct buffer_head *bh)
{
	disable_int();
	while(bh->b_lock)
		sleep_on(&bh->b_wait);
...
PRIVATE inline void unlock_buffer(struct buffer_head *bh)
{
	if(!bh->b_lock)
...
PRIVATE struct buffer_head * find_buffer(int dev,int block)
{
...
PRIVATE struct buffer_head * get_hash_buffer(int dev,int block)
{
...
PUBLIC void buffer_init(long buffer_start,long buffer_end)
{
